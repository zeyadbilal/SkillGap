...
2. **On any exception (Playwright not installed, page failed, still challenged) or a 0-row result, fall back automatically to `job_apis.fetch_jobs_for_track()`.** This hits two public job-board APIs that need no login, no key, and show no verification wall at all:
   ...
   Neither API lets you ask for "Network Security jobs" — they return their latest postings across every category, completely unlabelled, so every posting has to be assigned to *at most one* of the 8 tracks. `job_apis.py` classifies each posting once, across all 8 tracks together, in two tiers:
   1. **Title hints first.** A small set of near-unambiguous phrases per track (`"devops"`, `"ai engineer"`, `"full stack"`, `"network administrator"`, ...) is checked against the job title. If exactly one track's hints match, that title is trusted outright — a posted job title is written by the poster to say what the role actually is, which is a stronger signal than any keyword count.
   2. **Keyword-count fallback, mutually exclusive.** If the title is ambiguous or generic, every track's distinct-keyword hit count against `config/skills_dictionary.json` is computed, and the posting goes to whichever track scores highest — but only if that score clears `MIN_SKILL_HITS` (4). Exact ties are broken toward the more specific track (`TRACK_PRIORITY`): Full-Stack Development and DevOps & Cloud Engineering keep the broadest keyword lists (they legitimately share most of Backend/Frontend/Network Administration's own vocabulary plus extra terms), so on a tie the narrower track wins instead of whichever dictionary happens to be bigger.

   An earlier version of this classifier tested each track independently against the whole pool, so one posting (e.g. an "AI Engineer" role mentioning Docker/Azure/Python) could clear the bar for DevOps *and* ML/AI *and* Full-Stack at once and get duplicated into all three — that's the bug this two-tier, one-posting-one-track design fixes; it was caught by spotting a "Full Stack Engineer" and an "AI Engineer" posting inside `clean_jobs_devops_cloud_engineering.csv`. The two APIs are fetched once per `refresh_all_tracks()` cycle and the pool is shared across all 8 tracks (`job_apis.reset_pool_cache()`), not re-fetched per track.
...
- **The free-API fallback has uneven coverage across tracks, and it's now precision-first.** DevOps & Cloud Engineering and Machine Learning / AI reliably surface a healthy number of correctly-classified postings per cycle; Backend, Frontend, Full-Stack, Network Administration and Network Security typically surface a handful; Mobile Development can legitimately come back empty on a given cycle (mobile-specific roles are just rare on these two boards). Coverage was traded down deliberately in favor of not mislabeling: an earlier, looser version of the classifier produced more rows per track but let unrelated postings (an "AI Engineer" role, a "Full Stack Engineer" role) leak into `clean_jobs_devops_cloud_engineering.csv` — see [Data sources](#data-sources-wuzzuf-free-apis-manual-csv) for the fix. Seed a track with a manual CSV if the forecasting model needs denser data than the APIs currently provide for it.
- **The classifier is still keyword/title matching, not semantic understanding.** Title hints and the `MIN_SKILL_HITS`-4 keyword fallback with priority tie-breaks were tuned against one live snapshot of both APIs and verified to produce zero cross-track duplicates on it (every posting lands in at most one track's clean file) — but a posting with an unusual title and sparse, generic wording can still be dropped entirely (excluded from every track) rather than guessed at. That's the intended failure mode: no row is better than a mislabeled row.
...
| `fetch=0` / very few rows from `--source api` | Normal, especially for Mobile Development, Network Administration, or Network Security on a quiet cycle — the classifier drops anything it can't confidently assign rather than guessing, see [Known limitations](#known-limitations-read-this-before-trusting-the-numbers). Also check your network can reach `remoteok.com` and `arbeitnow.com`. |
| A posting for the wrong role ends up in a track's clean file | Should no longer happen — `job_apis._classify()` assigns each posting to at most one track. If you spot one anyway, it's a real bug: check `TITLE_HINTS` / `TRACK_PRIORITY` in `src/job_apis.py` for a gap and open an issue rather than editing the CSV by hand (it gets regenerated every refresh). |
