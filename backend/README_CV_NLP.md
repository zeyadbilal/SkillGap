...
- backend/src/services/pyNlpService.js — runs the Python script using conda, provides timeouts, and returns parsed extractor output or an error code.
...
- Conda (Miniconda/Anaconda) if using the Python extractor. The extractor can also run inside a Docker container.
- Recommended conda env name: myproject (but CV_NLP_CONDA_ENV env var can override).

Recommended env variables
-------------------------
- CV_NLP_CONDA_ENV (default: myproject) — conda environment name used to run the Python extractor.
...
2. Create conda env and install Python deps (example):
...
Behavior notes
--------------
- The service prefers the Python extractor when USE_PYTHON_NLP=true and the extractor is available. If it times out or fails, the service falls back to the JS extractor.
...
- To test the Python extractor in isolation, run:
  echo "<sample CV text>" | conda run -n myproject python model/nlp/spacy_extractor.py
...
2. Install conda in CI: use a GitHub Actions step that installs Miniconda, creates the env, installs packages, and downloads models. Example snippet:
...
3. Containerize the extractor into a small Docker image (recommended for reproducibility): build an image with conda and spaCy preinstalled and invoke it from Node instead of calling conda run.

Troubleshooting & common issues
-------------------------------
- "conda: command not found": ensure Miniconda/Anaconda is installed or use the Docker image option.
