...
function matchesKeyword(text, keyword) {
  ...
  return buildSkillAliases(keyword).some((alias) => phraseExists(searchableText, alias));
}
...
function inferTrack(detectedSkills) {
  ...
  return ranked[0] && ranked[0][1] > 0 ? ranked[0][0] : 'Full-Stack Development';
}

function getMarketSkills(track, limit = 10) {
  const marketData = loadMarketData();
